...
summary: Vue의 의지를 이어받아 모달을 구현해요
tag: { label: '개발', path: '/dev' }
---
...
> form에서의 상태관리 : 보통 formic이나 react-hook-form을 통해 관리하지만, 그런 라이브러리를 쓰지 않고 한다면 form 컴포넌트에서 모든 상태를 관리하고 하위 컴포넌트로 상위 컴포넌트의 상태를 변경할 수 있는 함수를 props로 내려주는게 일반적입니다.
...
const BookDetail = forwardRef<BookDetailModalRef, BookDetailModalProps>((props, ref) => {
	return <></>;
});
```
