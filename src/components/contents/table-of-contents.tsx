...
export default function TableOfContents({ headings }: TableOfContentsProps) {
	...
	return (
		...
		>
			<div
				className="flex max-w-[300px] flex-col rounded-lg border bg-white/80 p-4 shadow-lg backdrop-blur-sm dark:border-gray-700 dark:bg-gray-900/80"
				style={{ maxHeight: 'calc(100vh - 240px)' }}
			>
				<h3 className="mb-3 flex-shrink-0 text-sm font-semibold text-gray-900 dark:text-white">
					목차
				</h3>
				<nav className="flex-1 space-y-1 overflow-y-auto pr-1">
					{headings.map(heading => (
						<button
							...
							className={cn(
								'block w-full truncate text-left text-sm transition-colors hover:text-blue-600 dark:hover:text-blue-400',
								{
									'font-medium text-blue-600 dark:text-blue-400': activeId === heading.slug,
